...
export const BASE_PATH = "https://api.upload.io".replace(/\/+$/, "");
  ...
  get apiKey(): ((name: string) => string) | undefined {
    ...
    if (apiKey) {
      return typeof apiKey === "function"
        ? apiKey
        : (name: string): string => (name === "Authorization" ? `Bearer ${apiKey}` : apiKey);
    }
    return undefined;
  }
...
export class BaseAPI {
  ...
  protected async request(
    ...
    operationBasePathOverride: string | undefined
  ): Promise<Response> {
    const { url, init } = await this.createFetchParams(context, initOverrides, operationBasePathOverride);
    ...
    if (response !== undefined) {
      ...
      if (typeof jsonError?.error?.code === "string") {
        throw new UploadApiError(jsonError as ErrorResponse);
      }
    }
  ...
  private async createFetchParams(
    ...
    let url = (operationBasePathOverride ?? this.configuration.basePath) + context.path;
